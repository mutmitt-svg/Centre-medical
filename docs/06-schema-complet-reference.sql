...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION cs.unaccent_i(text) RETURNS text
...
CREATE INDEX idx_patient_nom_trgm ON patient USING gin ((cs.unaccent_i(nom || ' ' || coalesce(post_nom,'') || ' ' || coalesce(prenom,''))) gin_trgm_ops);
...
-- =====================================================================
-- FIN DU SCHÉMA — 136 tables, 6 vues d'agrégation, 4 déclencheurs métier, 21 types
-- Adaptations SQLite (poste isolé) :
--   uuid -> TEXT, jsonb -> TEXT, timestamptz -> TEXT ISO-8601,
--   ENUM -> TEXT + CHECK, GENERATED -> calcul applicatif,
--   pg_trgm -> table d'index phonétique dédiée.
-- =====================================================================
