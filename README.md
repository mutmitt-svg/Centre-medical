...
Cette application utilise le schéma `public`, déjà exposé par défaut dans Data API : il n'y a rien
à déclarer côté Supabase pour cette étape.

### Compte administrateur principal — depuis l'écran de connexion

Tant qu'aucun compte n'existe sur la base, l'écran de connexion affiche automatiquement un
formulaire de création du compte administrateur principal (nom complet, e-mail, mot de passe). Il
crée à la fois l'identité Supabase Auth et le dossier applicatif qui lui est rattaché, avec le rôle
`ADMIN` (toutes les permissions).

Si le projet Supabase exige la confirmation de l'adresse e-mail (réglage par défaut d'un nouveau
projet, dans Authentication > Providers > Email > « Confirm email »), un e-mail de confirmation est
envoyé : cliquez sur le lien reçu, puis reconnectez-vous avec cet identifiant et ce mot de passe —
le compte applicatif se termine automatiquement à cette première connexion. Cette étape ne se
produit qu'une seule fois : une fois un compte principal créé, ce formulaire n'apparaît plus et
l'écran de connexion habituel s'affiche pour tout le monde.

### Comptes additionnels — depuis le menu Administration

Une fois connecté avec le compte administrateur, le menu **Administration** propose un formulaire
« Créer un compte » (visible seulement pour un profil ayant la permission `UTILISATEUR_GERER`) :
nom, qualification, rôle applicatif, identifiant de connexion, e-mail et mot de passe temporaire à
communiquer à l'agent. Aucune manipulation dans le tableau de bord Supabase ni en SQL Editor n'est
nécessaire pour ces comptes suivants.

Ce formulaire appelle une fonction Edge (`supabase/functions/creer-utilisateur`), qui s'exécute
côté serveur avec des droits élevés pour créer l'identité Supabase Auth sans jamais exposer la clé
`service_role` au navigateur, et sans remplacer la session de l'administrateur en cours. Elle doit
être déployée une fois :

```
npx supabase functions deploy creer-utilisateur
```

La clé `service_role` nécessaire à cette fonction est fournie automatiquement par Supabase à
l'exécution : aucun secret à configurer manuellement pour cette étape.
...
```sql
select utilisateur_courant(), structure_courante(), permissions_courantes();
```

Si l'application affiche « Aucun compte applicatif rattaché à cette identité », c'est le signe
d'un compte Supabase Auth sans dossier `utilisateur` relié — normalement impossible avec le flux
ci-dessus, sauf en cas de compte Supabase Auth créé manuellement dans le tableau de bord sans passer
par l'écran de connexion ou le menu Administration.
