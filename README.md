...
```sql
select count(*) as tables     from pg_tables  where schemaname = 'cs';        -- 136
select count(*) as vues       from pg_views   where schemaname = 'cs';        -- 6
select count(*) as fonctions  from pg_proc p join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'cs';                                                     -- 27
select count(*) as politiques from pg_policies where schemaname = 'cs';       -- 236
```
...
| --- | --- | --- |
| `Failed to load module script: … MIME type of "text/jsx"` | un serveur statique sert le dossier des sources au lieu de la version compilée | servez `distribution/`, ou lancez `npm run dev` pour le développement |
...
| `permission denied for schema cs` | droits non appliqués | rejouez `20260922120500_rls_politiques.sql`, qui contient les autorisations |
