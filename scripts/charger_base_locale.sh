...
psql "$PGURL" -c "SELECT count(*) AS tables FROM pg_tables WHERE schemaname = 'public';"
psql "$PGURL" -c "SELECT count(*) AS fonctions FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace WHERE n.nspname = 'public';"
