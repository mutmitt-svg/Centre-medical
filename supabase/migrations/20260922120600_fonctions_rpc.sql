...
CREATE OR REPLACE FUNCTION public.age_years(p_date date) RETURNS int
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.rechercher_patient(p_q text, p_limite int DEFAULT 20)
RETURNS TABLE (
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH base AS (
    SELECT p.*, v.nom AS village_nom,
           similarity(public.unaccent_i(lower(concat_ws(' ', p.nom, p.post_nom, p.prenom))),
                      public.unaccent_i(lower(p_q))) AS sim
    FROM public.patient p
    LEFT JOIN public.village v ON v.id = p.village_id
    WHERE p.structure_id = public.structure_courante()
  ...
  SELECT id, numero_dossier,
         ...
         CASE
           ...
           WHEN public.age_years(date_naissance) < 1
             THEN (extract(month FROM age(current_date, date_naissance))::int)::text || ' mois'
           ELSE public.age_years(date_naissance)::text || ' ans'
         END AS age_affiche,
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.doublons_probables(
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT p.id, p.numero_dossier,
         ...
         round(similarity(public.unaccent_i(lower(concat_ws(' ', p.nom, p.post_nom, p.prenom))),
                          public.unaccent_i(lower(concat_ws(' ', p_nom, p_post_nom, p_prenom))))::numeric, 3)
    FROM public.patient p
   WHERE p.structure_id = public.structure_courante()
     AND p.deleted_at IS NULL
     AND p.sexe = p_sexe
     AND similarity(public.unaccent_i(lower(concat_ws(' ', p.nom, p.post_nom, p.prenom))),
                    public.unaccent_i(lower(concat_ws(' ', p_nom, p_post_nom, p_prenom)))) > 0.45
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.calculer_cas(p_patient uuid, p_date timestamptz DEFAULT now())
RETURNS cas_t
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT CASE
    WHEN EXISTS (
...
CREATE OR REPLACE FUNCTION public.enregistrer_venue(
  p_patient uuid,
  p_type type_venue_t,
...
DECLARE
  v_structure uuid := public.structure_courante();
  v_cas       cas_t;
  v_numero    text;
  v_jeton     smallint;
  v_regime    uuid;
...
BEGIN
  ...
  v_cas := coalesce(p_cas, public.calculer_cas(p_patient));
  ...
  INSERT INTO public.venue (structure_id, patient_id, numero_venue, numero_jeton, type_venue,
  ...
  VALUES (v_structure, p_patient, v_numero, v_jeton, p_type, v_cas, p_priorite, p_motif,
          p_service_id, p_refere_par, v_regime, public.utilisateur_courant())
  RETURNING * INTO v_row;

  PERFORM public.journaliser('CREATION', 'venue', v_row.id, p_patient);
  RETURN v_row;
END;
$$;
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.file_attente(p_service uuid DEFAULT NULL)
RETURNS TABLE (
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT v.id, v.numero_jeton, v.numero_venue, p.id,
         ...
         CASE WHEN p.date_naissance IS NULL THEN coalesce(p.age_declare_annees::text || ' ans','—')
              WHEN public.age_years(p.date_naissance) < 1
                THEN extract(month FROM age(current_date, p.date_naissance))::int::text || ' mois'
              ELSE public.age_years(p.date_naissance)::text || ' ans' END,
         p.sexe, v.type_venue, v.cas, v.priorite, v.statut, v.motif,
         (extract(epoch FROM (now() - v.date_heure_arrivee)) / 60)::int,
         pr.type_regime
    FROM public.venue v
    JOIN public.patient p ON p.id = v.patient_id
    LEFT JOIN public.patient_regime pr ON pr.id = v.regime_applique_id
   WHERE v.structure_id = public.structure_courante()
     AND v.deleted_at IS NULL
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.proposer_lots_fefo(
...
BEGIN
  FOR r IN
    SELECT s.lot_id, l.numero_lot, l.date_peremption, s.quantite
      FROM public.stock s JOIN public.lot l ON l.id = s.lot_id
...
CREATE OR REPLACE FUNCTION public.donnees_essentielles_sigl(p_periode date DEFAULT date_trunc('month', current_date)::date)
RETURNS TABLE (
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH d AS (SELECT id FROM public.depot WHERE structure_id = public.structure_courante()),
  sdu AS (
  ...
  mouv AS (
    SELECT l.produit_id,
      ...
      FROM public.mouvement_stock m JOIN public.lot l ON l.id = m.lot_id
  ...
  rupt AS (
    SELECT produit_id, coalesce(sum(nb_jours), 0)::int AS jours
      FROM public.rupture_stock
     WHERE structure_id = public.structure_courante()
...
CREATE OR REPLACE FUNCTION public.dispenser(
...
BEGIN
  ...
  FOR l IN SELECT * FROM jsonb_array_elements(p_lignes) LOOP
    ...
    SELECT * INTO v_lot FROM public.lot WHERE id = (l->>'lot_id')::uuid;
    ...
    SELECT lot_id INTO v_lot_fefo
      FROM public.proposer_lots_fefo(v_presc.produit_id, p_depot, v_qte) LIMIT 1;
    ...
    SELECT t.montant INTO v_montant
      FROM public.tarif t
      LEFT JOIN public.venue v ON v.id = p_venue
      LEFT JOIN public.patient_regime pr ON pr.id = v.regime_applique_id
    ...
    INSERT INTO public.dispensation (structure_id, venue_id, patient_id, prescription_id,
                                 depot_id, lot_id, quantite, montant, dispense_par)
    VALUES (public.structure_courante(), p_venue, v_patient, v_presc.id, p_depot,
            v_lot.id, v_qte, coalesce(v_montant, 0) * v_qte, public.utilisateur_courant())
    RETURNING id INTO v_disp;

    INSERT INTO public.mouvement_stock (structure_id, depot_id, lot_id, type, quantite,
                                    solde_apres, dispensation_id, derogation_fefo,
                                    motif_derogation, saisi_par)
    VALUES (public.structure_courante(), p_depot, v_lot.id, 'SORTIE', v_qte, 0, v_disp,
            v_derog, l->>'motif_derogation', public.utilisateur_courant());

    UPDATE public.prescription SET statut = 'DISPENSEE' WHERE id = v_presc.id;
  ...
  PERFORM public.journaliser('CREATION', 'dispensation', p_venue, v_patient);
END;
$$;
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.generer_facture(p_venue uuid)
RETURNS uuid
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_venue    public.venue;
  v_regime   public.patient_regime;
  v_taux     numeric := 0;
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.facture_ligne (facture_id, type_ligne, acte_id, libelle, quantite,
  ...
  FOR r IN SELECT del.id, del.examen_id, e.libelle
             FROM public.demande_examen_ligne del
             JOIN public.demande_examen de ON de.id = del.demande_id
             JOIN public.examen_ref e ON e.id = del.examen_id
  ...
  LOOP
    SELECT montant INTO v_pu FROM public.tarif
     WHERE examen_id = r.examen_id AND type_regime = 'PAYANT'
       AND date_debut <= current_date AND (date_fin IS NULL OR date_fin >= current_date)
     ORDER BY date_debut DESC LIMIT 1;
    INSERT INTO public.facture_ligne (facture_id, type_ligne, examen_id, libelle, quantite,
                                  prix_unitaire, taux_prise_charge, montant_total,
                                  part_patient, part_tiers)
  ...
  FOR r IN SELECT d.id, d.quantite, d.montant, p.dci, p.id AS produit_id
             FROM public.dispensation d
             JOIN public.prescription pr ON pr.id = d.prescription_id
             JOIN public.produit p ON p.id = pr.produit_id
            WHERE d.venue_id = p_venue AND d.facture_ligne_id IS NULL
  LOOP
    INSERT INTO public.facture_ligne (facture_id, type_ligne, produit_id, libelle, quantite,
                                  prix_unitaire, taux_prise_charge, montant_total,
                                  part_patient, part_tiers)
  ...
  FROM (SELECT coalesce(sum(montant_total), 0) AS brut,
          ...
          FROM public.facture_ligne WHERE facture_id = v_facture) t
  WHERE f.id = v_facture;

  PERFORM public.journaliser('CREATION', 'facture', v_facture, v_venue.patient_id);
  RETURN v_facture;
END;
$$;

CREATE OR REPLACE FUNCTION public.encaisser(
  p_facture uuid, p_session uuid, p_mode mode_paiement_t,
  p_devise devise_t, p_montant numeric, p_reference text DEFAULT NULL
) RETURNS public.paiement
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_facture public.facture;
  v_taux    numeric := 1;
  v_numero  text;
  v_row     public.paiement;
  v_paye    numeric;
BEGIN
  SELECT * INTO v_facture FROM public.facture WHERE id = p_facture;
  ...
  IF p_devise = 'USD' THEN
    SELECT taux INTO v_taux FROM public.taux_change
  ...
  SELECT to_char(now(), 'YYYYMM') || '-' || lpad((count(*) + 1)::text, 5, '0')
    INTO v_numero FROM public.paiement
  ...
  INSERT INTO public.paiement (structure_id, facture_id, session_caisse_id, numero_recu, mode,
  ...
  VALUES (v_facture.structure_id, p_facture, p_session, v_numero, p_mode, p_devise,
          p_montant, v_taux, p_montant * v_taux, p_reference, public.utilisateur_courant())
  RETURNING * INTO v_row;

  SELECT coalesce(sum(montant_equiv_cdf), 0) INTO v_paye
    FROM public.paiement WHERE facture_id = p_facture AND NOT annule;
  ...
  PERFORM public.journaliser('CREATION', 'paiement', v_row.id, v_facture.patient_id);
  RETURN v_row;
END;
$$;

CREATE OR REPLACE FUNCTION public.cloturer_session_caisse(
  p_session uuid, p_compte_cdf numeric, p_compte_usd numeric DEFAULT 0,
  p_justification text DEFAULT NULL
) RETURNS public.session_caisse
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_th_cdf numeric; v_th_usd numeric; v_row public.session_caisse;
  v_ecart_cdf numeric; v_ecart_usd numeric; v_fonds numeric;
BEGIN
  SELECT fonds_initial INTO v_fonds FROM public.session_caisse WHERE id = p_session;
  ...
  PERFORM public.journaliser('MODIFICATION', 'session_caisse', p_session, NULL, p_justification);
  RETURN v_row;
END;
$$;
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.calendrier_vaccinal(p_patient uuid)
RETURNS TABLE (antigene text, libelle text, numero_dose smallint,
               date_theorique date, date_recue date, statut text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH p AS (SELECT date_naissance FROM public.patient WHERE id = p_patient)
  SELECT a.code, a.libelle, a.numero_dose,
         ...
         CASE
           ...
           ELSE 'DU' END
    FROM public.antigene a
    LEFT JOIN public.vaccination v ON v.antigene_id = a.id AND v.patient_id = p_patient
   WHERE a.actif
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.generer_rapport_snis(
...
BEGIN
  SELECT id, version INTO v_rapport, v_version
    FROM public.rapport_mensuel
   WHERE structure_id = v_structure AND periode_mois = v_mois AND type_rapport = p_type
   ORDER BY version DESC LIMIT 1;

  IF v_rapport IS NOT NULL THEN
    IF (SELECT statut FROM public.rapport_mensuel WHERE id = v_rapport) IN ('TRANSMIS','VALIDE') THEN
      -- RG-11 : une période transmise est figée, on ouvre un rectificatif versionné
      INSERT INTO public.rapport_mensuel (structure_id, periode_mois, type_rapport, statut,
                                      version, genere_par)
      VALUES (v_structure, v_mois, p_type, 'RECTIFIE', v_version + 1, public.utilisateur_courant())
      RETURNING id INTO v_rapport;
    ELSE
      DELETE FROM public.rapport_valeur WHERE rapport_id = v_rapport;
      UPDATE public.rapport_mensuel SET genere_le = now(), genere_par = public.utilisateur_courant()
       WHERE id = v_rapport;
    END IF;
  ELSE
    INSERT INTO public.rapport_mensuel (structure_id, periode_mois, type_rapport, statut,
                                    version, genere_par)
    VALUES (v_structure, v_mois, p_type, 'BROUILLON', 1, public.utilisateur_courant())
    RETURNING id INTO v_rapport;
  END IF;

  FOR v_elem IN SELECT id, code FROM public.element_donnee_snis WHERE actif LOOP
    v_val := NULL;
    CASE v_elem.code
      WHEN 'SNIS-CONS-001' THEN
        SELECT count(*) INTO v_val FROM public.venue
      ...
      WHEN 'SNIS-CONS-005' THEN
        SELECT count(*) INTO v_val FROM public.venue v
          JOIN public.patient_regime pr ON pr.id = v.regime_applique_id
      ...
      WHEN 'SNIS-MERE-001' THEN
        SELECT count(*) INTO v_val FROM public.visite_cpn c
          JOIN public.dossier_grossesse g ON g.id = c.dossier_grossesse_id
      ...
      WHEN 'SNIS-MERE-003' THEN
        SELECT count(*) INTO v_val FROM public.travail_accouchement
      ...
      WHEN 'SNIS-ENF-001' THEN
        SELECT count(*) INTO v_val FROM public.vaccination v JOIN public.antigene a ON a.id = v.antigene_id
      ...
      WHEN 'SNIS-LAB-001' THEN
        SELECT count(*) INTO v_val FROM public.resultat_examen r
          JOIN public.demande_examen_ligne dl ON dl.id = r.demande_ligne_id
          JOIN public.demande_examen d ON d.id = dl.demande_id
          JOIN public.venue ve ON ve.id = d.venue_id
    ...
    IF v_val IS NOT NULL THEN
      INSERT INTO public.rapport_valeur (rapport_id, element_id, ventilation_cle,
                                     valeur_calculee, valeur_retenue)
      VALUES (v_rapport, v_elem.id, 'TOTAL', v_val, v_val);
    END IF;
  END LOOP;

  UPDATE public.rapport_mensuel r
     SET complet = (
       SELECT count(*) FROM public.rapport_valeur rv WHERE rv.rapport_id = r.id
     ) >= (
       SELECT count(*) FROM public.element_donnee_snis WHERE actif AND obligatoire
     )
   WHERE r.id = v_rapport;

  PERFORM public.journaliser('CREATION', 'rapport_mensuel', v_rapport);
  RETURN v_rapport;
END;
$$;

CREATE OR REPLACE FUNCTION public.controles_coherence(p_rapport uuid)
...
BEGIN
  SELECT rv.valeur_retenue INTO v FROM public.rapport_valeur rv
    JOIN public.element_donnee_snis e ON e.id = rv.element_id
   WHERE rv.rapport_id = p_rapport AND e.code = 'SNIS-CONS-001';
  SELECT rv.valeur_retenue INTO n FROM public.rapport_valeur rv
    JOIN public.element_donnee_snis e ON e.id = rv.element_id
   WHERE rv.rapport_id = p_rapport AND e.code = 'SNIS-CONS-002';
  SELECT rv.valeur_retenue INTO a FROM public.rapport_valeur rv
    JOIN public.element_donnee_snis e ON e.id = rv.element_id
   WHERE rv.rapport_id = p_rapport AND e.code = 'SNIS-CONS-003';
  SELECT rv.valeur_retenue INTO cpn1 FROM public.rapport_valeur rv
    JOIN public.element_donnee_snis e ON e.id = rv.element_id
   WHERE rv.rapport_id = p_rapport AND e.code = 'SNIS-MERE-001';
  SELECT rv.valeur_retenue INTO cpn4 FROM public.rapport_valeur rv
    JOIN public.element_donnee_snis e ON e.id = rv.element_id
   WHERE rv.rapport_id = p_rapport AND e.code = 'SNIS-MERE-002';
  SELECT rv.valeur_retenue INTO acc FROM public.rapport_valeur rv
    JOIN public.element_donnee_snis e ON e.id = rv.element_id
   WHERE rv.rapport_id = p_rapport AND e.code = 'SNIS-MERE-003';
  SELECT rv.valeur_retenue INTO deces FROM public.rapport_valeur rv
    JOIN public.element_donnee_snis e ON e.id = rv.element_id
...
CREATE OR REPLACE FUNCTION public.transmettre_rapport(
  p_rapport uuid, p_canal text DEFAULT 'DHIS2_API'
) RETURNS public.rapport_mensuel
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_row public.rapport_mensuel; v_bloquant int;
BEGIN
  SELECT count(*) INTO v_bloquant
    FROM public.controles_coherence(p_rapport)
  ...
  UPDATE public.rapport_mensuel
     SET statut = 'TRANSMIS', transmis_le = now(), canal_transmission = p_canal,
         valide_par = public.utilisateur_courant(), date_validation = now()
   WHERE id = p_rapport
  RETURNING * INTO v_row;

  PERFORM public.journaliser('EXPORT', 'rapport_mensuel', p_rapport);
  RETURN v_row;
END;
$$;
...
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE
  s uuid := public.structure_courante();
  res jsonb;
BEGIN
  SELECT jsonb_build_object(
    'consultations_jour', (SELECT count(*) FROM public.venue
       WHERE structure_id = s AND date_heure_arrivee::date = current_date AND deleted_at IS NULL),
    'nouveaux_cas_jour', (SELECT count(*) FROM public.venue
       WHERE structure_id = s AND cas = 'NOUVEAU' AND date_heure_arrivee::date = current_date),
    'en_attente', (SELECT count(*) FROM public.venue
       WHERE structure_id = s AND statut NOT IN ('CLOTUREE','ABANDON')
         AND date_heure_arrivee::date = current_date),
    'recettes_jour_cdf', (SELECT coalesce(sum(montant_equiv_cdf), 0) FROM public.paiement
       WHERE structure_id = s AND date_heure::date = current_date AND NOT annule),
    'traceurs_total', (SELECT count(*) FROM public.produit WHERE traceur AND actif),
    'traceurs_disponibles', (SELECT count(DISTINCT l.produit_id)
       FROM public.stock st JOIN public.lot l ON l.id = st.lot_id
       JOIN public.produit p ON p.id = l.produit_id
       JOIN public.depot d ON d.id = st.depot_id
      WHERE d.structure_id = s AND p.traceur AND st.quantite > 0
        AND l.date_peremption > current_date),
    'peremptions_90j', (SELECT count(*) FROM public.stock st JOIN public.lot l ON l.id = st.lot_id
       JOIN public.depot d ON d.id = st.depot_id
      WHERE d.structure_id = s AND st.quantite > 0
        AND l.date_peremption BETWEEN current_date AND current_date + 90),
    'resultats_critiques', (SELECT count(*) FROM public.resultat_examen r
       JOIN public.demande_examen_ligne dl ON dl.id = r.demande_ligne_id
       JOIN public.demande_examen de ON de.id = dl.demande_id
       JOIN public.venue ve ON ve.id = de.venue_id
      WHERE ve.structure_id = s AND r.critique AND r.date_heure_resultat::date >= current_date - 2),
    'accouchements_mois', (SELECT count(*) FROM public.travail_accouchement
    ...
    'dernier_rapport', (SELECT jsonb_build_object('periode', periode_mois, 'statut', statut)
       FROM public.rapport_mensuel WHERE structure_id = s AND type_rapport = 'SNIS_CS'
      ORDER BY periode_mois DESC LIMIT 1),
    'population', (SELECT population_totale FROM public.population_annuelle
